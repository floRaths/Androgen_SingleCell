...
input_vargenes <- celltype

adjacencies <- read_tsv(paste0("~/Documents/SCENIC/pySCENIC_Full/scenicdata/CT_All_4kvar/Results/", input_vargenes, "/expr_mat.adjacencies.tsv"))
...
GRN_Scores_raw <- 
  readRDS(paste0("~/Documents/SCENIC/pySCENIC_Full/scenicdata/CT_All_4kvar/GRN_Scores/Percentile_99/GRN_Scores_99_perc_", input_vargenes, ".rds"))  %>% 
  dplyr::select(-c(1:15))

GRN_Scores <- 
  readRDS(paste0("~/Documents/SCENIC/pySCENIC_Full/scenicdata/CT_All_4kvar/GRN_Scores/Percentile_99/GRN_Scores_99_perc_", input_vargenes, ".rds"))  %>% 
  dplyr::select(-c(1:15))#%>% column_to_rownames("Cell")
...
#Sobj_plot   <- AddMetaData(subset(Sobj_integrated, idents = celltype), metadata = GRN_Scores_raw)

MetaData    <- readRDS("Output/MetaData_May05.rds")

Wilcox_bind <- GRN_Scores %>%
  rownames_to_column("Cell") %>% 
  left_join(dplyr::select(rownames_to_column(MetaData, "Cell"), Cell, Sample, Type), by = "Cell") %>% 
  pivot_longer(cols = colnames(GRN_Scores), names_to = "TF") %>% 
  group_by(Sample, TF) %>% 
  mutate(Sample.Mean = median(value)) %>% 
  dplyr::select(-1) %>% 
  distinct() %>%
  group_by(TF) %>% do(w = wilcox.test(value~Type, data = ., paired=FALSE)) %>% 
  summarise(TF, Wilcox = w$p.value) %>% filter(Wilcox <= 0.05) %>% arrange(Wilcox) %>% add_column(CellType = input_vargenes)



Marker_Bind <- do.call(rbind.data.frame, readRDS(paste0("Output/TM-CF_Response_", category, "_MAST_list.rds"))) %>% 
  filter(p_val_adj <= 0.05) %>% group_by(Gene) %>% mutate(events = n()) %>% arrange(-events)
...
GRN_Summary <<- 
  GRN_Scores %>% dplyr::select(celltype.tfs) %>%                        # take the significant tfs and the cells
  rownames_to_column("Cell") %>%                                                      
  left_join(dplyr::select(rownames_to_column(MetaData, "Cell"), # join with metadat (this can be clenaed up)
                          Cell, Type, Sample), by = "Cell") %>%
  group_by(Sample, Type) %>% summarise_at(vars(celltype.tfs), median) %>%          # calculate mean regulon scores per Sample
  pivot_longer(cols = celltype.tfs, names_to = "TF") %>% 
  group_by(Type, TF) %>%
  mutate(mean.out = median(value), is_outlier = !isnt_out_tukey(value)) %>%        # detect outliers within their groups and calculate mean of whole Type
  group_by(Type, TF, is_outlier) %>%                                             # separate outliers and non-outliers
  mutate(mean.in = median(value)) %>%                                              # calculate mean without outliers
  ...
  arrange(-change) %>% left_join(Wilcox_bind, by = "TF")                                                               # which allows us to select strong candicted
...
GRN_Samp_Summ <<- 
  GRN_Scores %>% dplyr::select(celltype.tfs) %>%
  rownames_to_column("Cell") %>% 
  left_join(dplyr::select(rownames_to_column(MetaData, "Cell"), Cell, Sample), by = "Cell") %>%
  group_by(Sample) %>% summarise_at(vars(celltype.tfs), median)
...
curve <- 
GRN_Summary %>%
  ggplot(aes(x = reorder(TF, change), y = change, color = -log10(Wilcox), size = -log10(Wilcox))) + 
  ...
  geom_hline(yintercept = 0, color = "grey") + scale_color_viridis()
...
VlnPlot(Sobj_plot, idents = celltype, features = c("TP63_1"), ncol = 1, pt.size = 0, split.by = "Type", group.by = "Sample")
...
GRN_Scores %>%
  ...
  pivot_longer(cols = colnames(GRN_Scores), names_to = "TF") %>% 
  group_by(Sample, TF) %>% 
  mutate(Sample.Mean = mean(value)) %>% 
  dplyr::select(-1, -value) %>% 
  distinct() %>%
  group_by(TF) %>% do(w = wilcox.test(Sample.Mean~Type, data = ., paired=FALSE)) %>% 
  summarise(TF, Wilcox = w$p.value) %>% filter(Wilcox <= 0.05) %>% arrange(Wilcox)
