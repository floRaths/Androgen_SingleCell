...
CellRanger_to_Sobj  <- function() {
  ...
  for (i in seq_along(samples)) {
    rawdata_list[[i]] <- Read10X            (data.dir = file.path("/home/rathsf/scNuclei-Libraries/cellrangered/2021_CellRanger_ReRun/", samples[i] , "/outs/filtered_feature_bc_matrix/"))
...
AssignClusterIdents <- function() {
  ...
  CellType_Markers_Final <- read_excel("CellType_Markers_Final.xlsx")
  ...
  names(newIdent) <- Assign$RNA_snn_res.0.05
...
DoubletDetection    <- function() {
  ...
  for (i in 1:length(samples)) {
    p[i] <- paste0("/home/rathsf/scNuclei-Libraries/cellrangered/2021_CellRanger_ReRun/", samples[i], "/outs/filtered_feature_bc_matrix/")
  }
...
GenerateMetaData    <- function() {
  ...
  LibraryMetrics <- read_excel("utilities/2021_ReAnalysis/LibraryMetrics.xlsx", skip = 0, sheet = 1)
...
samples <- samples <- samples[-c(13, 21)]
samples.short <- substr(samples, 1, nchar(samples)-8)
samples.short <- samples

levels <- c("LUM_HR-pos", "LUM_HR-neg", "Basal", "Fibroblast", "Adipocyte", "Blood_EC", "Lymph_EC", "Vasc.Acc.", "Lymphoid", "Myeloid", "HSC")
DietSeurat(Sobj, counts = T, data = T, scale.data = F, assays = "RNA", dimreducs = c("pca", "harmony", "umap"))
### read in 10X data and create a merged Seurat object with unique barcodes
CellRanger_to_Sobj()


Sobj <- readRDS("2021_ReRun/Sobj_Final_Scaled_Harmony_Meta_V2.rds")
...
Sobj <- subset(Sobj, subset = percent.mt < 2.5 & nCount_RNA < 20000 & scDblFinder.class == "singlet") # adjust percent.mito and nCount RNA according to VlnPlots
...
VlnPlot(Sobj, features = "nCount_RNA", group.by  = "RNA_snn_res.0.05") + ylim(c(0, 5000))
...
Sobj <- readRDS("Seurat_Objects/5_Final_Sobj_Scaled_Harmony_Meta/Sobj_Final_Scaled_Harmony_Meta_V2.rds")
...
for (i in 1:length(levels(Sobj))) {
  ...
  subset %>% 
    ...
    saveRDS(paste0("Seurat_Objects/CellType_Sobjs/2_After_Manual_CleanUp/1000_Vargenes/", levels(Sobj)[i], ".rds"))

}


DimPlot(Sobj, group.by = "RNA_snn_res.0.2", label = T, split.by = "Type")
DimPlot(Sobj, split.by = "Sample", label = T, ncol = 5)
VlnPlot(Sobj, features = "nCount_RNA", group.by = "RNA_snn_res.0.2") + ylim(c(1, 10000))

FeaturePlot(Sobj, features = c("ANKRD30A", "TP63", "KIT", "nCount_RNA", "VWF", "LAMA2"), cols = viridis(n = 100, option = "A"), pt.size = 1, order = T)

Sobj@meta.data %>% select(nCount_RNA, RNA_snn_res.0.2) %>% group_by(RNA_snn_res.0.2) %>% summarise(mean = mean(nCount_RNA))
